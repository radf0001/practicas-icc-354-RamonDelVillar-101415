...
public class AdminController {
    ...
    @GetMapping(value="/")
    public ModelAndView indexAdmin(Authentication authentication, HttpSession session) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/usuario")
    public ModelAndView crearUsuarioGet(Authentication authentication, @RequestParam Optional<Integer> id, HttpSession session)
    {
        String idSesion = session.getId();
...
//        System.out.println(usuario);
        ...
        if (id.isEmpty()){
            if(f)
            {
                ...
                usuario.setRoles(new HashSet<>(Collections.singletonList(seguridadServices.rolFindById(usuario.getRol()))));
            ...
            if(temp != null) {
                ...
                if (f && !temp.getUsername().equalsIgnoreCase(usuario.getUsername())) {
                    ...
                    temp.setRoles(new HashSet<>(Collections.singletonList(seguridadServices.rolFindById(usuario.getRol()))));
                    Usuario user;
    ...
    public ModelAndView mocks(Authentication authentication, @RequestParam Optional<Integer> id, HttpSession session) throws UnknownHostException {
        ...
        params.put("puerto", environment.getProperty("local.server.port"));
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/mock")
    public ModelAndView crearMockGet(Authentication authentication, @RequestParam Optional<Integer> id, HttpSession session)
    {
